...
\usepackage{caption}
% Sets caption text to small and Sans Serif
% Sets 'Figure 1:' label to small, Bold, and Sans Serif
\captionsetup[figure]{
    font={small,sf}, 
    labelfont={bf,sf},
    skip=10pt % adds space between figure and caption
}
\usepackage{amsmath}
\usepackage{booktabs}
...
% Define the cover image here
\titlepic{\includegraphics[width=0.6\textwidth]{figures/data_flow_cover_v010.png}}

\begin{document}

% --- TITLE PAGE (No Numbering) ---
\maketitle
\thispagestyle{empty} % Removes number from first page only
\newpage

% --- ABSTRACT (Roman Numbering) ---
\pagenumbering{roman} 
\begin{abstract}
...
% --- INTRODUCTION (Arabic Numbering) ---
\pagenumbering{arabic}
\setcounter{page}{1} % Resets counter to 1
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{The Minimum Viable Disclosure Interface pipeline showing the transition from internal state to audit artifacts.}
...
\begin{table}[htbp]
    ...
    \small
    \begin{tabularx}{\textwidth}{l X r} % Left-aligned, expanding middle, right-aligned
        \toprule
...
\begin{figure}[htbp] % <--- Ensure this opening tag is here
    \centering
    \vspace{1em}
    ...
    \vspace{1em} % Add space before the caption
    
    \caption*{Figure 1: \textbf{The Glasshouse Dilemma.} Visualizing information resistance as the divide between internal state and deterministic audit artifacts (Equation \ref{eq:shannon_entropy}). The prototype achieves a zero-entropy baseline (Equation \ref{eq:baseline_val}).}
\end{figure} % <--- Matches the opening tag
...
Within the scope of the preliminary release, public contributions to KTH Royal Institute of Technology and the CHAINS research project about "Consistent Hardening and Analysis of Software Supply Chains" \cite{chains-project} could not be verified as evidence for the baseline. Specific external repositories are therefore not referred to as results in Section~\ref{sec:reproducibility}. Adjacent projects stemming from KTH Rymdcenter and other Space Agencies are treated as access-gated candidates. Mist Lab in Montreal, Canada \cite{mistlab}, is noted only as a possible research target for the proposed methodology, not as traceability for the existing framework. Integration in the future enhanced system is contingent upon formal recording of the repository URL, access permissions, commit hashes, licensing, and defined audit scope.
...
\bibitem{arxiv-submit}

[arXiv](https://blog.arxiv.org/2026/01/13/)

.
\newblock Submission Guidelines.
\newblock \url{https://info.arxiv.org/help/submit/index.html}.
...
\bibitem{chains-project}
\newblock CHAINS research project 
\newblock \url{https://github.com/chains-project}.
\newblock Accessed May 2026.

\bibitem{mistlab}
\newblock Mist Lab.
\newblock \url{https://github.com/mistlab}.
\newblock Accessed May 2026.
